...
function Move(player, iDrag, dest)
	print("Item moved from " .. iDrag .. " to " .. dest)
...
function DropItem(player, slot)
	if(equipped[player][slot] ~= nil) then
			print("Dropped item")
		local droppedItemEquipment = equipped[player][slot]
		local droppedItemPickup = droppedItemEquipment:GetCustomProperty("PickupAssetMUID")
		droppedItemEquipment:Unequip()
		if(Object.IsValid(droppedItemEquipment)) then
			print("Destroyed Dropped Item")
			droppedItemEquipment:Destroy()
		end
		World.SpawnAsset(droppedItemPickup, {parent=nil,position=player:GetWorldPosition()})
		equipped[player][slot] = nil
		data[player][slot] = nil
		Events.Broadcast("inventorySaveEvent", player, data[player])
	end
end

function ClearPlayerInventory(player)
	playerData = Storage.GetPlayerData(player) or {}
	playerData.inventory = {}
	Storage.SetPlayerData(player, playerData)
end

Game.playerJoinedEvent:Connect(ClearPlayerInventory)

function StoreInventoryData(player, data)
	playerData = Storage.GetPlayerData(player) or {}
	playerData.inventory = data
	Storage.SetPlayerData(player, playerData)
end

Events.Connect("inventorySaveEvent", StoreInventoryData)
Events.Connect("dropItemEvent", DropItem)
...
function MoveSave(player, iDrag, dest)
	Events.Broadcast("inventorySaveEvent", player, data[player])
end

Events.Connect("inventoryMoveEvent", MoveSave)
-------------------------------------------------------------
