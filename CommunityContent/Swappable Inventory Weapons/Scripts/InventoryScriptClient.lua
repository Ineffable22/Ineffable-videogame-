...
function MoveItemImage(fromIndex, toIndex)

	--Remove any images in the "toIndex" slot
	if(#buttons[toIndex]:FindDescendantsByType("UIImage") > 0) then
		for _, element in ipairs(buttons[toIndex]:FindDescendantsByType("UIImage")) do
			element:Destroy()
			element = nil
		end
	end

	--Only moves the first child image
	print("Moved Image from: " .. fromIndex .. " to: " .. toIndex)
	local child = buttons[fromIndex]:FindDescendantByType("UIImage")
	local image = child
	image.parent = buttons[toIndex]
	image.x = 0
	image.y = 0
end

Events.Connect("moveItemImage", MoveItemImage)
-------------------------------------------------------------------------------------